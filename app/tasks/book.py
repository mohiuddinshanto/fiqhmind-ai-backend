...
them into a per-book task graph:
    ...
    extraction → chunking → group(embed, index)
...
endpoints call instead of leaf tasks directly. `fail_book_jobs` is wired as
each leaf's `link_error` errback so a raising child fails every queued/in-flight
job of the book instead of leaving them stuck; permanent child failures (which
the leaf tasks record without raising) keep the existing job-status semantics.
"""
...
def build_book_graph(
    *,
    ...
    """The full per-book graph: extraction → chunking → group(embed, index)."""
    return chain(
